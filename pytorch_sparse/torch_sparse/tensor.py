...
class SparseTensor(object):
    ...
    def set_csc(self):
        ...
        if value is not None:
            value = value[perm]
        self.storage._cscvalue = value
        self.storage._rowidx = self.storage.row()[perm]
        return colptr

    def get_csc(self) -> Tuple[torch.Tensor, Optional[torch.Tensor], 
                               Optional[torch.Tensor]]:
        if self.storage._cscvalue is not None:
            cscvalue = self.storage._cscvalue
        else:
            cscvalue = None
        if self.storage._rowidx is None:
            print("Error: you should use set_csc() before get_csc().")
        assert self.storage._rowidx is not None
        return self.storage.colptr(), self.storage._rowidx, cscvalue

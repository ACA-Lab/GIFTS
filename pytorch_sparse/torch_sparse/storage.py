...
class SparseStorage(object):
    ...
    _cscvalue: Optional[torch.Tensor]
    _rowidx: Optional[torch.Tensor]
    ...
    def __init__(
        ...
        cscvalue: Optional[torch.Tensor] = None,
        rowidx: Optional[torch.Tensor] = None,
    ...
    def clone(self):
        ...
        cscvalue = self._cscvalue
        if cscvalue is not None:
            cscvalue = cscvalue.clone()
        rowidx = self._rowidx
    ...
    def to_device(self, device: torch.device, non_blocking: bool = False):
        ...
        cscvalue = self._cscvalue
        if cscvalue is not None:
            cscvalue = cscvalue.to(device, non_blocking=non_blocking)
        rowidx = self._rowidx
        if rowidx is not None:
            rowidx = rowidx.to(device, non_blocking=non_blocking)
    ...
    def cuda(self):
        ...
        # Not implement col-wise in cuda
        return SparseStorage(
            row=row,
            rowptr=rowptr,
            col=new_col,
    ...
    def pin_memory(self):
        ...
        cscvalue = self._cscvalue
        if cscvalue is not None:
            cscvalue = cscvalue.pin_memory()
        rowidx = self._rowidx
        if rowidx is not None:
            rowidx = rowidx.pin_memory()

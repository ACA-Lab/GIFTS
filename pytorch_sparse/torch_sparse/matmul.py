...
def spmm_openmp(src: SparseTensor,
         other: torch.Tensor, reduce: str = "sum") -> torch.Tensor:
    if reduce == 'sum' or reduce == 'add' or reduce == 'mean':
        rowptr, colidx, value = src.csr()

        if value is not None:
            value = value.to(other.dtype)
        
        return torch.ops.torch_sparse.spmm_openmp(rowptr, colidx, value, other, reduce)
    else:
        raise ValueError

def spmm_reorder(src: SparseTensor,
         other: torch.Tensor, Reorder_seq: torch.Tensor,
         reduce: str = "sum") -> torch.Tensor:
    if reduce == 'sum' or reduce == 'add' or reduce == 'mean':
        rowptr, colidx, value = src.csr()
        ...
        return torch.ops.torch_sparse.spmm_reorder(rowptr, colidx, value, other, Reorder_seq, reduce)
    else:
        raise ValueError

def spmm_compress(src: SparseTensor,
         other: torch.Tensor, CompressTuple,
         reduce: str = "sum") -> torch.Tensor:
    if reduce == 'sum' or reduce == 'add' or reduce == 'mean':
        rowptr, colidx, value = src.csr()
        ...
        return torch.ops.torch_sparse.spmm_compress(rowptr, colidx, value, other, CompressTuple, reduce)
    else:
        raise ValueError
    
def spmm_compress_value(src: SparseTensor,
         other: torch.Tensor, CompressTuple,
         reduce: str = "sum") -> torch.Tensor:
    if reduce == 'sum' or reduce == 'add' or reduce == 'mean':
        rowptr, colidx, value = src.csr()
        ...
        return torch.ops.torch_sparse.spmm_compress_value(rowptr, colidx, value, other, CompressTuple, reduce)
    else:
        raise ValueError

def spmm_reorder_compress(src: SparseTensor,
         other: torch.Tensor, Reorder_seq: torch.Tensor,
         CompressTuple,
         reduce: str = "sum") -> torch.Tensor:
    if reduce == 'sum' or reduce == 'add' or reduce == 'mean':
        ...
        return torch.ops.torch_sparse.spmm_reorder_compress(rowptr, colidx, value, other, Reorder_seq, CompressTuple, reduce)
    else:
        raise ValueError
...
def matmul(src, other, reduce="sum", Reorder_seq=None, CompressTuple=None, Value_only=False):  # noqa: F811
    ...
    if isinstance(other, torch.Tensor):
        if Reorder_seq is not None:
            print("Use reorder")
            if CompressTuple is not None:
                print("Use Compress")
                assert Value_only==False, "Value only support is not available in Scheduling"
                return spmm_reorder_compress(src, other, Reorder_seq, CompressTuple, reduce)
            return spmm_reorder(src, other, Reorder_seq, reduce)
        if CompressTuple is not None:
            print("Use Compress")
            if Value_only:
                print("Value Only")
                return spmm_compress_value(src, other, CompressTuple, reduce)
            return spmm_compress(src, other, CompressTuple, reduce)
        # return spmm_openmp(src, other, reduce)

...
template <typename scalar_t, ReductionType REDUCE, bool HAS_VALUE>
__global__ void spmm_compress_kernel(const int64_t *rowptr_data, const int64_t *col_data,
                            const scalar_t *value_data,
                            const scalar_t *mat_data, scalar_t *out_data,
                            int64_t *arg_out_data, int B, int M, int N, int K) {
...
template <typename scalar_t, ReductionType REDUCE, bool HAS_VALUE>
__global__ void spmm_reorder_kernel(const int64_t *bal_seq, const int64_t *rowptr_data, const int64_t *col_data,
                            const scalar_t *value_data,
                            const scalar_t *mat_data, scalar_t *out_data,
                            int64_t *arg_out_data, int B, int M, int N, int K) {
...
template <typename scalar_t, ReductionType REDUCE, bool HAS_VALUE>
__global__ void spmm_compress_reorder_kernel(const int64_t *bal_seq, const int64_t *rowptr_data, const int64_t *col_data,
                            const scalar_t *value_data,
                            const scalar_t *mat_data, scalar_t *out_data,
                            int64_t *arg_out_data, int B, int M, int N, int K) {
...
spmm_compress_cuda(torch::Tensor rowptr, torch::Tensor col,
  ...
  AT_DISPATCH_ALL_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, mat.scalar_type(), "_", [&] {
    auto mat_data = mat.data_ptr<scalar_t>();
    auto out_data = out.data_ptr<scalar_t>();
    ...
    AT_DISPATCH_REDUCTION_TYPES(reduce, [&] {
      if (optional_value.has_value()) {
        auto value_data = optional_value.value().data_ptr<scalar_t>();
        spmm_compress_kernel<scalar_t, REDUCE, true><<<BLOCKS, THREADS, 0, stream>>>(
            rowptr_data, col_data, value_data, mat_data, out_data, arg_out_data,
            B, M, N, K);
      } else {
        spmm_compress_kernel<scalar_t, REDUCE, false><<<BLOCKS, THREADS, 0, stream>>>(
            rowptr_data, col_data, nullptr, mat_data, out_data, arg_out_data, B,
            M, N, K);
      }
    });
...
spmm_reorder_cuda(torch::Tensor reorder_seq, torch::Tensor rowptr, torch::Tensor col,
  ...
  auto rowptr_data = rowptr.data_ptr<int64_t>();
  auto col_data = col.data_ptr<int64_t>();
  auto reorder_seq_data = reorder_seq.data_ptr<int64_t>();
  ...
  AT_DISPATCH_ALL_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, mat.scalar_type(), "_", [&] {
    ...
    AT_DISPATCH_REDUCTION_TYPES(reduce, [&] {
      if (optional_value.has_value()) {
        auto value_data = optional_value.value().data_ptr<scalar_t>();
        spmm_reorder_kernel<scalar_t, REDUCE, true><<<BLOCKS, THREADS, 0, stream>>>(
            reorder_seq_data, rowptr_data, col_data, value_data, mat_data, out_data, arg_out_data,
            B, M, N, K);
      } else {
        spmm_reorder_kernel<scalar_t, REDUCE, false><<<BLOCKS, THREADS, 0, stream>>>(
            reorder_seq_data, rowptr_data, col_data, nullptr, mat_data, out_data, arg_out_data, B,
            M, N, K);
      }
    });
  });
...
spmm_compress_reorder_cuda(torch::Tensor reorder_seq, torch::Tensor rowptr, torch::Tensor col,
  ...
  AT_DISPATCH_ALL_TYPES_AND(at::ScalarType::Half, mat.scalar_type(), "_", [&] {
    ...
    AT_DISPATCH_REDUCTION_TYPES(reduce, [&] {
      if (optional_value.has_value()) {
        auto value_data = optional_value.value().data_ptr<scalar_t>();
        spmm_compress_reorder_kernel<scalar_t, REDUCE, true><<<BLOCKS, THREADS, 0, stream>>>(
            reorder_seq_data, rowptr_data, col_data, value_data, mat_data, out_data, arg_out_data,
            B, M, N, K);
      } else {
        spmm_compress_reorder_kernel<scalar_t, REDUCE, false><<<BLOCKS, THREADS, 0, stream>>>(
            reorder_seq_data, rowptr_data, col_data, nullptr, mat_data, out_data, arg_out_data, B,
            M, N, K);
      }
    });
